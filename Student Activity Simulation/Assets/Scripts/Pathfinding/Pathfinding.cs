...
public class Pathfinding {
    ...
    public Pathfinding(int width, int height, int cellSize, Vector3 startPosition) {
        Instance = this;
        grid = new Grid<PathNode>(width, height, cellSize, startPosition, (Grid<PathNode> g, int x, int y) => new PathNode(g, x, y));
    }
    ...
    public List<Vector3> FindPath(Vector3 startWorldPosition, Vector3 endWorldPosition) {
        ...
        if (path == null) {
            ...
            Debug.Log("YES PATH FOUND");
            List<Vector3> vectorPath = new List<Vector3>();
            foreach (PathNode pathNode in path) {
                vectorPath.Add(new Vector3(pathNode.x * grid.GetCellSize(), pathNode.y * grid.GetCellSize()) + (Vector3.one * grid.GetCellSize() * .5f) + grid.GetOriginPosition());
            }
            return vectorPath;
        }
    }
